...
impl SlashCommand {
    ...
    pub fn execute(
        ...
        match self {
            ...
            Self::Cost { window } => CommandResult::ShowCost {
                window: window.clone(),
            },
            Self::Skills => {
                let invocable: Vec<_> = known_skills.iter().filter(|s| s.user_invocable).collect();
            ...
            Self::Commit { message } => CommandResult::Commit {
                message: message.clone(),
            },
            Self::CommitPushPr { message } => CommandResult::CommitPushPr {
                message: message.clone(),
            },
            Self::Pr { prompt } => CommandResult::Pr {
                prompt: prompt.clone(),
            },
            Self::Bug { prompt } => CommandResult::Bug {
                prompt: prompt.clone(),
            },
            Self::Search { query } => CommandResult::Search {
                query: query.clone(),
            },
            ...
            Self::Export { format } => CommandResult::Export {
                format: format.clone(),
            },
            ...
            Self::Fast { toggle } => CommandResult::Fast {
                toggle: toggle.clone(),
            },
...
pub(crate) fn resolve_command_result(
    input: &str,
    cwd: &Path,
    known_skills: &[SkillEntry],
) -> Option<CommandResult> {
    let loader = PluginLoader::discover(cwd);
    let plugin_commands = plugin_command_entries(&loader);
    let parsed = SlashCommand::parse(input, known_skills)?;
    let resolved = resolve_plugin_command(parsed, &loader);

    Some(match resolved {
        Ok(command) => command.execute(known_skills, &plugin_commands),
        Err(message) => CommandResult::Print(message),
    })
}

fn plugin_command_entries(loader: &PluginLoader) -> Vec<PluginCommandEntry> {
    loader
        .all_commands()
        .into_iter()
        .map(|(plugin, command)| PluginCommandEntry {
...
fn resolve_plugin_command(
    ...
    if let SlashCommand::Unknown(ref name) = command {
        if let Some((plugin, plugin_command)) = loader
            .all_commands()
            .into_iter()
            .find(|(_, plugin_command)| plugin_command.name == *name)
        {
            let prompt = PluginLoader::command_prompt(plugin, plugin_command)
                .ok_or_else(|| format!("Plugin command /{name} has no prompt file."))?;
            return Ok(SlashCommand::RunPluginCommand {
                name: name.clone(),
                prompt,
            });
        }
...
fn build_help_text(skills: &[SkillEntry], plugin_commands: &[PluginCommandEntry]) -> String {
    ...
    if !plugin_commands.is_empty() {
        ...
        for pc in plugin_commands {
            text.push_str(&format!(
                "\n  /{:<20} (from {})",
                pc.command_name, pc.plugin_name
            ));
        }
    }
    text
...
mod tests {
    ...
    fn no_skills() -> Vec<SkillEntry> {
        Vec::new()
    }
    fn no_plugins() -> Vec<PluginCommandEntry> {
        Vec::new()
    }
    ...
    fn test_parse_basic_commands() {
        ...
        assert!(matches!(
            SlashCommand::parse("/clear", &s),
            Some(SlashCommand::Clear)
        ));
        assert!(matches!(
            SlashCommand::parse("/exit", &s),
            Some(SlashCommand::Exit)
        ));
        assert!(matches!(
            SlashCommand::parse("/quit", &s),
            Some(SlashCommand::Exit)
        ));
        assert!(matches!(
            SlashCommand::parse("/version", &s),
            Some(SlashCommand::Version)
        ));
        assert!(matches!(
            SlashCommand::parse("/diff", &s),
            Some(SlashCommand::Diff)
        ));
        assert!(matches!(
            SlashCommand::parse("/status", &s),
            Some(SlashCommand::Status)
        ));
        assert!(matches!(
            SlashCommand::parse("/undo", &s),
            Some(SlashCommand::Undo)
        ));
        assert!(matches!(
            SlashCommand::parse("/doctor", &s),
            Some(SlashCommand::Doctor)
        ));
        assert!(matches!(
            SlashCommand::parse("/init", &s),
            Some(SlashCommand::Init)
        ));
        assert!(matches!(
            SlashCommand::parse("/login", &s),
            Some(SlashCommand::Login)
        ));
        assert!(matches!(
            SlashCommand::parse("/logout", &s),
            Some(SlashCommand::Logout)
        ));
        assert!(matches!(
            SlashCommand::parse("/cost", &s),
            Some(SlashCommand::Cost { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/skills", &s),
    ...
    fn test_parse_case_insensitive() {
        ...
        assert!(matches!(
            SlashCommand::parse("/Model sonnet", &s),
    ...
    fn test_parse_aliases() {
        let s = no_skills();
        assert!(matches!(
            SlashCommand::parse("/perms", &s),
            Some(SlashCommand::Permissions { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/permissions", &s),
            Some(SlashCommand::Permissions { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/ctx", &s),
            Some(SlashCommand::Context)
        ));
        assert!(matches!(
            SlashCommand::parse("/context", &s),
            Some(SlashCommand::Context)
        ));
        assert!(matches!(
    ...
    fn test_execute_compact_with_instructions() {
        let cmd = SlashCommand::Compact {
            instructions: "focus on code".into(),
        };
        match cmd.execute(&no_skills(), &no_plugins()) {
    ...
    fn test_parse_bug() {
        let s = no_skills();
        assert!(matches!(
            SlashCommand::parse("/bug login broken", &s),
            Some(SlashCommand::Bug { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/debug crash", &s),
            Some(SlashCommand::Bug { .. })
        ));
    }
    ...
    #[test]
    fn test_execute_pr() {
        let cmd = SlashCommand::Pr {
            prompt: "review security".into(),
        };
        match cmd.execute(&no_skills(), &no_plugins()) {
    ...
    #[test]
    fn test_execute_bug() {
        let cmd = SlashCommand::Bug {
            prompt: "OOM crash".into(),
        };
        match cmd.execute(&no_skills(), &no_plugins()) {
    ...
    fn test_parse_search() {
        ...
        // aliases
        assert!(matches!(
            SlashCommand::parse("/find foo", &s),
            Some(SlashCommand::Search { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/grep bar", &s),
            Some(SlashCommand::Search { .. })
        ));
    }
    ...
    fn test_execute_commit_push_pr() {
        let cmd = SlashCommand::CommitPushPr {
            message: "new feature".into(),
        };
        match cmd.execute(&no_skills(), &no_plugins()) {
    ...
    fn test_help_text_includes_plugin_commands() {
        let plugins = vec![PluginCommandEntry {
    ...
    fn test_parse_retry() {
        let s = no_skills();
        assert!(matches!(
            SlashCommand::parse("/retry", &s),
            Some(SlashCommand::Retry)
        ));
    }
    ...
    fn test_parse_all_aliases() {
        ...
        // /config aliases
        assert!(matches!(
            SlashCommand::parse("/config", &s),
            Some(SlashCommand::Config)
        ));
        assert!(matches!(
            SlashCommand::parse("/settings", &s),
            Some(SlashCommand::Config)
        ));
        // /branch aliases
        assert!(matches!(
            SlashCommand::parse("/branch feat", &s),
            Some(SlashCommand::Branch { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/fork feat", &s),
            Some(SlashCommand::Branch { .. })
        ));
        // /pr-comments aliases
        assert!(matches!(
            SlashCommand::parse("/pr-comments 42", &s),
            Some(SlashCommand::PrComments { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/prc 42", &s),
            Some(SlashCommand::PrComments { .. })
        ));
        // /reload-context aliases
        assert!(matches!(
            SlashCommand::parse("/reload-context", &s),
            Some(SlashCommand::ReloadContext)
        ));
        assert!(matches!(
            SlashCommand::parse("/reload", &s),
            Some(SlashCommand::ReloadContext)
        ));
        // /plugin aliases
        assert!(matches!(
            SlashCommand::parse("/plugin", &s),
            Some(SlashCommand::Plugin { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/plugins", &s),
            Some(SlashCommand::Plugin { .. })
        ));
        // /agents aliases
        assert!(matches!(
            SlashCommand::parse("/agents", &s),
            Some(SlashCommand::Agents { .. })
        ));
        assert!(matches!(
            SlashCommand::parse("/agent", &s),
            Some(SlashCommand::Agents { .. })
        ));
    }
    ...
    fn test_execute_permissions() {
        let cmd = SlashCommand::Permissions {
            mode: "bypass".into(),
        };
        match cmd.execute(&no_skills(), &no_plugins()) {
    ...
    fn test_execute_login_logout() {
        ...
        assert!(matches!(
            login.execute(&no_skills(), &no_plugins()),
            CommandResult::Login
        ));
        let logout = SlashCommand::Logout;
        assert!(matches!(
            logout.execute(&no_skills(), &no_plugins()),
    ...
    #[test]
    fn test_resolve_command_result_includes_plugin_commands_in_help() {
        ...
        match resolve_command_result("/help", tmp.path(), &no_skills()) {
            Some(CommandResult::Print(text)) => {
                assert!(text.contains("/greet"));
                assert!(text.contains("from cmd-plugin"));
            }
    ...
    #[test]
    fn test_resolve_command_result_runs_plugin_command() {
        let tmp = TempDir::new().unwrap();
        let plugins_dir = tmp.path().join(".claude").join("plugins");
        create_plugin_dir(
            &plugins_dir,
        ...
        match resolve_command_result("/greet", tmp.path(), &no_skills()) {
            Some(CommandResult::RunPluginCommand { name, prompt }) => {
                assert_eq!(name, "greet");
                assert_eq!(prompt, "Greet the user");
            }
    ...
    #[test]
    fn test_resolve_command_result_reports_missing_plugin_prompt() {
        let tmp = TempDir::new().unwrap();
        let plugins_dir = tmp.path().join(".claude").join("plugins");
        create_plugin_dir(
            &plugins_dir,
            ...
            r#"{
                "name": "cmd-plugin",
                "commands": [
                    {"name": "greet", "description": "Say hello", "promptFile": "prompts/greet.md"}
                ]
            }"#,
        );
        ...
        match resolve_command_result("/greet", tmp.path(), &no_skills()) {
            Some(CommandResult::Print(text)) => {
                assert!(text.contains("Plugin command /greet has no prompt file."));
            }
            _ => panic!("expected plugin prompt error"),
        }
    }
    ...
    fn test_parse_files() {
        ...
        // alias
        assert!(matches!(
            SlashCommand::parse("/ls", &s),
            Some(SlashCommand::Files { .. })
        ));
    }
    ...
    fn test_parse_env() {
        let s = no_skills();
        assert!(matches!(
            SlashCommand::parse("/env", &s),
            Some(SlashCommand::Env)
        ));
        assert!(matches!(
            SlashCommand::parse("/environment", &s),
            Some(SlashCommand::Env)
        ));
    }
    ...
    fn test_parse_vim() {
        let s = no_skills();
        assert!(
            matches!(SlashCommand::parse("/vim", &s), Some(SlashCommand::Vim { toggle }) if toggle.is_empty())
        );
        assert!(
            matches!(SlashCommand::parse("/vim on", &s), Some(SlashCommand::Vim { toggle }) if toggle == "on")
        );
        assert!(
            matches!(SlashCommand::parse("/vim off", &s), Some(SlashCommand::Vim { toggle }) if toggle == "off")
        );
    }
    ...
    fn test_parse_stickers() {
        let s = no_skills();
        assert!(matches!(
            SlashCommand::parse("/stickers", &s),
            Some(SlashCommand::Stickers)
        ));
    }
    ...
    fn test_parse_effort() {
        let s = no_skills();
        assert!(
            matches!(SlashCommand::parse("/effort", &s), Some(SlashCommand::Effort { level }) if level.is_empty())
        );
        assert!(
            matches!(SlashCommand::parse("/effort high", &s), Some(SlashCommand::Effort { level }) if level == "high")
        );
    }
    ...
    fn test_parse_tag() {
        let s = no_skills();
        assert!(
            matches!(SlashCommand::parse("/tag", &s), Some(SlashCommand::Tag { name }) if name.is_empty())
        );
        assert!(
            matches!(SlashCommand::parse("/tag important", &s), Some(SlashCommand::Tag { name }) if name == "important")
        );
    }
    ...
    fn test_parse_release_notes() {
        let s = no_skills();
        assert!(matches!(
            SlashCommand::parse("/release-notes", &s),
            Some(SlashCommand::ReleaseNotes)
        ));
        assert!(matches!(
            SlashCommand::parse("/changelog", &s),
            Some(SlashCommand::ReleaseNotes)
        ));
    }
    ...
    fn test_parse_feedback() {
        let s = no_skills();
        assert!(
            matches!(SlashCommand::parse("/feedback", &s), Some(SlashCommand::Feedback { text }) if text.is_empty())
        );
        assert!(
            matches!(SlashCommand::parse("/feedback great tool!", &s), Some(SlashCommand::Feedback { text }) if text == "great tool!")
        );
    }
    ...
    fn test_execute_effort_empty() {
        ...
        assert!(
            matches!(cmd.execute(&no_skills(), &no_plugins()), CommandResult::Effort { level } if level.is_empty())
        );
    }
    ...
    fn test_execute_feedback_with_text() {
        ...
        assert!(
            matches!(cmd.execute(&no_skills(), &no_plugins()), CommandResult::Feedback { text } if text == "hello")
        );
    }
