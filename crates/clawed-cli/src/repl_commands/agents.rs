...
fn show_live_status(tracker: Option<&AgentTracker>) {
    ...
    for task in &tasks {
        match task.status {
            clawed_agent::coordinator::AgentStatus::Running => running.push(task),
    ...
    if !finished.is_empty() {
        ...
        for task in &finished {
            ...
            let status_icon = match task.status {
                clawed_agent::coordinator::AgentStatus::Completed => "\x1b[32m✓\x1b[0m",
                clawed_agent::coordinator::AgentStatus::Failed => "\x1b[31m✗\x1b[0m",
                clawed_agent::coordinator::AgentStatus::Killed => "\x1b[33m⊘\x1b[0m",
                clawed_agent::coordinator::AgentStatus::Running => "▸", // shouldn't reach here
            };
            println!(
