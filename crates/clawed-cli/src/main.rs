...
async fn run() -> anyhow::Result<()> {
    ...
    if let Some(ref query) = cli.search_sessions {
        let sessions = clawed_core::session::search_sessions(query);
        if sessions.is_empty() {
    ...
    // RUST_LOG takes priority (e.g. RUST_LOG=clawed_api=trace for raw packets)
    ...
    let model_input = if cli.model == "claude-sonnet-4-20250514" && cli.provider != "anthropic" {
        clawed_core::model::default_model_for_provider(&cli.provider).to_string()
    } else {
        cli.model.clone()
    };
    ...
    let model = if cli.base_url.is_some() {
        ...
        clawed_core::model::validate_model_for_provider(&model_input, &cli.provider)
    ...
    let engine = clawed_agent::engine::QueryEngine::builder(api_key, &cwd)
        .model(&model)
        .system_prompt(system_prompt)
        .max_turns(cli.max_turns)
        .permission_checker(clawed_agent::permissions::PermissionChecker::new(
            permission_mode,
    ...
    let mcp_adapter = clawed_mcp::McpBusAdapter::new(mcp_manager);

    // Create the core adapter bridging QueryEngine ↔ EventBus
    let adapter = clawed_agent::bus_adapter::AgentCoreAdapter::from_arc(
        Arc::clone(&engine),
        bus_handle,
        Some(mcp_adapter),
    );
