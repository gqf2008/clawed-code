...
mod tests {
    ...
    #[test]
    fn version_override() {
        assert_eq!(agent_info(Some("2.0"))["version"], "2.0");
    }
    #[test]
    fn caps_exist() {
        assert!(agent_capabilities().get("promptCapabilities").is_some());
    }
    #[test]
    fn init_ok() {
        assert_eq!(initialize_response(Some("1"))["protocolVersion"], 1);
    }
    #[test]
    fn rpc_ok() {
        let r = rpc_result(&json!(1), json!({}));
        assert_eq!(r["jsonrpc"], "2.0");
    }
    #[test]
    fn rpc_err() {
        let e = rpc_error(&json!("a"), -1, "err");
        assert_eq!(e["error"]["code"], -1);
    }
    #[test]
    fn extract() {
        let b: Vec<Value> = serde_json::from_value(json!([{"type":"text","text":"hi"}])).unwrap();
        assert_eq!(extract_text_from_content(&b), Some("hi\n".into()));
    }
    #[test]
    fn extract_empty() {
