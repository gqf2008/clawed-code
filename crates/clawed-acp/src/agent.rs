...
impl AcpAgent {
    ...
    pub async fn handle_close_session(&self, sid: &str) -> Result<()> {
        self.sessions.close_session(sid).await
    }
    pub async fn handle_list_sessions(&self) -> Value {
        json!({"sessions": self.sessions.list_sessions().await})
    }

    pub async fn handle_read_text_file(&self, path: &str) -> Result<Value> {
        Ok(
            json!({"content": tokio::fs::read_to_string(path).await.with_context(||format!("read {path}"))?}),
        )
    }
