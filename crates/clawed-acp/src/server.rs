...
impl AcpServer {
    ...
    pub fn new(
        ...
        Self {
            agent: Arc::new(AcpAgent::new(
                engine,
                Arc::new(McpAcpBridge::new(mcp_manager)),
            )),
        ...
        while let Some(line) = lines.next_line().await? {
            ...
            if !trimmed.is_empty() {
                if let Some(r) = self.dispatch(trimmed).await {
                    let mut j = serde_json::to_string(&r)?;
                    j.push('\n');
                ...
                while let Some(Ok(msg)) = read.next().await {
                    ...
                    if let Some(r) = server.dispatch(&text).await {
                        if let Ok(j) = serde_json::to_string(&r) {
                            let _ = write
                                .send(tokio_tungstenite::tungstenite::Message::Text(j))
                                .await;
                        }
                    }
                }
    ...
    pub(crate) async fn dispatch(&self, msg_str: &str) -> Option<Value> {
        ...
        let result = match method {
            ...
            "session/request_permission" => {
                Ok(json!({"outcome":"selected","optionId":"allow_once"}))
            }
            ...
            "terminal/kill" | "terminal/release" => {
                Self::handle_terminal_kill(params);
                Ok(json!({}))
            }
...
impl AcpServer {
    fn handle_new_session(&self, params: Option<&Value>) -> Result<Value> {
        ...
        let cwd = p
            .get("cwd")
            .and_then(|v| v.as_str())
            .ok_or(anyhow::anyhow!("Missing cwd"))?;
        let agent = self.agent.clone();
        let cwd = cwd.to_string();
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(agent.handle_new_session(&cwd))
        })
    }
    ...
    fn handle_set_config_option(&self, params: Option<&Value>) -> Result<Value> {
        ...
        let key = p
            .get("configId")
            .and_then(|v| v.as_str())
            .ok_or(anyhow::anyhow!("Missing configId"))?;
        let val = p
            .get("value")
            .and_then(|v| v.as_str())
        ...
        let res = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current()
                .block_on(async { agent.session_manager().set_config(&sid, &key, &val).await })
        });
        res?;
        Ok(tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current()
                .block_on(agent.session_manager().get_config_json(&sid))
        }))
    }
    ...
    fn handle_set_mode(&self, params: Option<&Value>) -> Result<Value> {
        ...
        let mid = p
            .get("modeId")
            .and_then(|v| v.as_str())
            .ok_or(anyhow::anyhow!("Missing modeId"))?;
        let agent = self.agent.clone();
        let sid = sid.to_string();
        let mid = mid.to_string();
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(agent.session_manager().set_mode(&sid, &mid))
        })?;
        Ok(json!({}))
    }
        ...
        match std::process::Command::new(cmd).spawn() {
            Ok(child) => {
                TERMINALS
                    ...
                    .insert(tid.clone(), child);
                Ok(json!({"terminalId": tid}))
            }
    ...
    fn handle_terminal_kill(params: Option<&Value>) {
        if let Some(tid) = params.and_then(|p| p.get("terminalId").and_then(|v| v.as_str())) {
            if let Some(mut child) = TERMINALS
                .lock()
                .unwrap_or_else(|e| e.into_inner())
                .remove(tid)
            {
    ...
    fn handle_terminal_output(params: Option<&Value>) -> Value {
        let tid = params
            .and_then(|p| p.get("terminalId").and_then(|v| v.as_str()))
            .map(|s| s.to_string());
    ...
    fn handle_mcp_connect(&self, params: Option<&Value>) -> Result<Value> {
        ...
        let id = p
            .get("acpId")
            .and_then(|v| v.as_str())
            .ok_or(anyhow::anyhow!("Missing acpId"))?;
        let agent = self.agent.clone();
        let id = id.to_string();
        let conn_id = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(agent.mcp_bridge().handle_connect(&id))
        })?;
    ...
    async fn handle_mcp_msg(&self, params: Option<&Value>) -> Result<Value> {
        ...
        let cid = p
            .get("connectionId")
            .and_then(|v| v.as_str())
            .ok_or(anyhow::anyhow!("Missing connectionId"))?;
        let method = p
            .get("method")
            .and_then(|v| v.as_str())
        ...
        self.agent
            .mcp_bridge()
            .handle_message(cid, method, p.get("params").cloned())
            .await
    }

    async fn handle_mcp_disconnect(&self, params: Option<&Value>) -> Result<Value> {
        let cid = params
            .and_then(|p| p.get("connectionId").and_then(|v| v.as_str()))
            .ok_or(anyhow::anyhow!("Missing connectionId"))?;
