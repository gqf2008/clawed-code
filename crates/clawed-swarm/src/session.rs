...
//!
//! Unlike `clawed-agent`'s full `QueryEngine`, this session is purpose-built
//! for swarm `AgentActor`s:
//! - No hooks, compaction, or coordinator
//! - Direct `clawed-api` calls with a built-in tool executor
//! - Conversation state stored in-actor (history Vec)
//!
//! # Circular dependency avoidance
//! `clawed-swarm` cannot depend on `clawed-agent` (which depends on `clawed-swarm`).
