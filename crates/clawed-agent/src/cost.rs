...
//!
//! Pricing is sourced from [`clawed_core::model::model_pricing`] — the single
...
use clawed_core::message::Usage;
use clawed_core::model;

/// Calculate USD cost from a `Usage` struct and model name.
///
/// Delegates to [`clawed_core::model::model_pricing`] for per-model rates.
