...
use clawed_bus::bus::BusHandle;
use clawed_bus::events::*;
use clawed_core::message::{ContentBlock, ImageSource};
    ...
    async fn handle_list_models(&self) {
        ...
        let models: Vec<ModelInfo> = aliases
            ...
            .chain(std::iter::once(ModelInfo {
                ...
                display_name: clawed_core::model::display_name_any(&current_model),
            }))
            .collect();
...
mod tests {
    ...
    fn usage_events_return_none() {
        let event = AgentEvent::UsageUpdate(clawed_core::message::Usage {
            input_tokens: 100,
            output_tokens: 50,
        ...
        match event {
            ...
            AgentEvent::AssistantMessage(msg) => Some(AgentNotification::AssistantMessage {
                ...
                text_blocks: msg
                    ...
                    .iter()
                    .filter_map(|b| match b {
                        clawed_core::message::ContentBlock::Text { text } => Some(text.clone()),
                        _ => None,
                    })
                    .collect(),
            }),
