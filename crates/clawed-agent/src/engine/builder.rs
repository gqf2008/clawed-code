...
use clawed_core::config::HooksConfig;
use clawed_core::memory::load_memories_for_prompt;
...
pub struct QueryEngineBuilder {
    ...
    pub(crate) thinking: Option<clawed_api::types::ThinkingConfig>,
    ...
    pub(crate) mcp_manager: Option<Arc<RwLock<clawed_mcp::McpManager>>>,
}
...
impl QueryEngineBuilder {
    ...
    pub fn thinking(mut self, config: Option<clawed_api::types::ThinkingConfig>) -> Self {
    ...
    pub fn mcp_manager(mut self, manager: Arc<RwLock<clawed_mcp::McpManager>>) -> Self {
    ...
    pub fn build(self) -> QueryEngine {
        ...
        if let Some(ref provider) = self.provider {
            if provider != "anthropic" {
                let backend = clawed_api::provider::create_backend(
                    provider,
                    &self.api_key,
        ...
        match clawed_computer_use::ComputerUseMcpServer::new() {
            Ok(cu_server) => {
                let cu_server = Arc::new(cu_server);
                let read_only_tools = &["screenshot", "cursor_position"];
                let proxies = clawed_tools::mcp::create_builtin_tool_proxies(
                    cu_server.as_ref(),
                    clawed_core::tool::ToolCategory::ComputerUse,
                    read_only_tools,
                    mcp_manager.clone(),
                );
        ...
        if std::env::var("CLAUDE_CODE_SWARM").unwrap_or_default() == "1" {
            ...
            clawed_swarm::register_swarm_tools(&mut registry, default_model, &default_cwd);
        }
        ...
        let memory_dir_str = if self.load_memory {
            clawed_core::memory::primary_memory_dir(&self.cwd)
