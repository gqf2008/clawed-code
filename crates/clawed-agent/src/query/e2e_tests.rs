...
fn make_stream_events(response: MessagesResponse) -> Vec<anyhow::Result<clawed_api::types::StreamEvent>> {
    ...
    for (idx, block) in content.iter().enumerate() {
        match block {
            ResponseContentBlock::Text { text } => {
                ...
                events.push(Ok(clawed_api::types::StreamEvent::ContentBlockStop { index: idx }));
            }
            ...
            ResponseContentBlock::Thinking { thinking } => {
                ...
                events.push(Ok(clawed_api::types::StreamEvent::ContentBlockDelta {
                    index: idx,
                    delta: clawed_api::types::DeltaBlock::ThinkingDelta { thinking: thinking.clone() },
                }));
                events.push(Ok(clawed_api::types::StreamEvent::ContentBlockStop { index: idx }));
            }
        }
    ...
    events.push(Ok(clawed_api::types::StreamEvent::MessageDelta {
        ...
        usage: Some(clawed_api::types::DeltaUsage { output_tokens: usage.output_tokens }),
    }));
...
fn test_setup(mock: MockBackend) -> (
    ...
    let client = Arc::new(
        clawed_api::client::ApiClient::new("test-key")
...
async fn e2e_full_tool_round_trip_with_registered_tool() {
    ...
    impl clawed_core::tool::Tool for EchoTool {
        ...
        async fn call(&self, input: serde_json::Value, _ctx: &clawed_core::tool::ToolContext) -> anyhow::Result<clawed_core::tool::ToolResult> {
            let text = input["text"].as_str().unwrap_or("(empty)");
            Ok(clawed_core::tool::ToolResult::text(format!("Echo: {}", text)))
        }
        fn is_read_only(&self) -> bool { true }
        fn category(&self) -> clawed_core::tool::ToolCategory { clawed_core::tool::ToolCategory::Session }
    }
...
async fn e2e_permission_denied_returns_error_tool_result() {
    ...
    impl clawed_core::tool::Tool for DeniedTool {
        ...
        async fn call(&self, _input: serde_json::Value, _ctx: &clawed_core::tool::ToolContext) -> anyhow::Result<clawed_core::tool::ToolResult> {
            Ok(clawed_core::tool::ToolResult::text("should not execute"))
        }
        fn is_read_only(&self) -> bool { false }
        fn category(&self) -> clawed_core::tool::ToolCategory { clawed_core::tool::ToolCategory::Shell }
    }
...
async fn e2e_text_and_tool_use_in_same_response() {
    ...
    impl clawed_core::tool::Tool for EchoTool2 {
        ...
        async fn call(&self, input: serde_json::Value, _ctx: &clawed_core::tool::ToolContext) -> anyhow::Result<clawed_core::tool::ToolResult> {
            Ok(clawed_core::tool::ToolResult::text(format!("Echo: {}", input["text"].as_str().unwrap_or(""))))
        }
        fn is_read_only(&self) -> bool { true }
        fn category(&self) -> clawed_core::tool::ToolCategory { clawed_core::tool::ToolCategory::Session }
    }
    ...
    let registry = Arc::new(registry);
    let perm = Arc::new(crate::permissions::PermissionChecker::new(
        clawed_core::permissions::PermissionMode::Default, vec![],
    ));
    let executor = Arc::new(crate::executor::ToolExecutor::new(registry, perm));
    let state = crate::state::new_shared_state();
    let tool_context = clawed_core::tool::ToolContext {
