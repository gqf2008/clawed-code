...
use clawed_bus::events::{AgentNotification, AgentRequest};
    ...
    match method {
        ...
        "agent.setModel" => {
            let p = params.ok_or_else(|| {
                RpcError::new(
                    error_codes::INVALID_PARAMS,
                    "Missing params for agent.setModel",
                )
            })?;
            let model = p
                .get("model")
        ...
        "agent.permission" => {
            let p = params.ok_or_else(|| {
                RpcError::new(
                    error_codes::INVALID_PARAMS,
                    "Missing params for agent.permission",
                )
            })?;
            let request_id = p
                .get("request_id")
        ...
        "session.load" => {
            let p = params.ok_or_else(|| {
                RpcError::new(
                    error_codes::INVALID_PARAMS,
                    "Missing params for session.load",
                )
            })?;
            let session_id = p
                .get("session_id")
        ...
        "mcp.connect" => {
            let p = params.ok_or_else(|| {
                RpcError::new(
                    error_codes::INVALID_PARAMS,
                    "Missing params for mcp.connect",
                )
            })?;
            let name = p
                .get("name")
            ...
            let args: Vec<String> = p
                ...
                .unwrap_or_default();
            let env: std::collections::HashMap<String, String> = p
                .get("env")
                .and_then(|v| serde_json::from_value(v.clone()).ok())
                .unwrap_or_default();
            Ok(AgentRequest::McpConnect {
                name,
                command,
                args,
                env,
...
pub fn notification_to_jsonrpc(notif: &AgentNotification) -> Notification {
    match notif {
        AgentNotification::TextDelta { text } => Notification::new(
            "agent.textDelta",
            json_obj(1, &[("text", Value::String(text.clone()))]),
        ),
        AgentNotification::ThinkingDelta { text } => Notification::new(
            "agent.thinkingDelta",
            json_obj(1, &[("text", Value::String(text.clone()))]),
        ),
        AgentNotification::ToolUseStart { id, tool_name } => Notification::new(
            "agent.toolStart",
            json_obj(
                2,
                &[
        ...
        AgentNotification::TurnStart { turn } => Notification::new(
            "agent.turnStart",
            json_obj(1, &[("turn", serde_json::json!(turn))]),
        ),
        ...
        AgentNotification::AssistantMessage { turn, text_blocks } => Notification::new(
            "agent.assistantMessage",
            Some(serde_json::json!({
                "turn": turn, "text_blocks": text_blocks
            })),
        ),
        AgentNotification::SessionStart { session_id, model } => Notification::new(
            "session.start",
            json_obj(
                2,
                &[
        ...
        AgentNotification::SessionEnd { reason } => Notification::new(
            "session.end",
            json_obj(1, &[("reason", Value::String(reason.clone()))]),
        ),
        AgentNotification::SessionSaved { session_id } => Notification::new(
            "session.saved",
            json_obj(1, &[("session_id", Value::String(session_id.clone()))]),
        ),
        ...
        AgentNotification::ContextWarning { usage_pct, message } => Notification::new(
            "agent.contextWarning",
            Some(serde_json::json!({
                "usage_pct": usage_pct, "message": message
            })),
        ),
        AgentNotification::CompactStart => Notification::new("agent.compactStart", None),
        AgentNotification::CompactComplete { summary_len } => Notification::new(
            "agent.compactComplete",
            json_obj(1, &[("summary_len", serde_json::json!(summary_len))]),
        ),
        ...
        AgentNotification::AgentProgress { agent_id, text } => Notification::new(
            "agent.progress",
            json_obj(
                2,
                &[
        ...
        AgentNotification::McpServerConnected { name, tool_count } => Notification::new(
            "mcp.connected",
            json_obj(
                2,
                &[
        ...
        AgentNotification::McpServerDisconnected { name } => Notification::new(
            "mcp.disconnected",
            json_obj(1, &[("name", Value::String(name.clone()))]),
        ),
        AgentNotification::McpServerError { name, error } => Notification::new(
            "mcp.error",
            json_obj(
                2,
                &[
        ...
        AgentNotification::McpServerList { servers } => {
            ...
            Notification::new(
                "mcp.serverList",
                json_obj(1, &[("servers", Value::Array(list))]),
            )
        }
        AgentNotification::MemoryExtracted { facts } => Notification::new(
            "agent.memoryExtracted",
            Some(serde_json::json!({ "facts": facts })),
        ),
        AgentNotification::ModelList { models } => {
            ...
            Notification::new(
                "agent.modelList",
                json_obj(1, &[("models", Value::Array(list))]),
            )
        }
        AgentNotification::ToolList { tools } => {
            ...
            Notification::new(
                "agent.toolList",
                json_obj(1, &[("tools", Value::Array(list))]),
            )
        }
        AgentNotification::Error { code, message } => Notification::new(
            "agent.error",
            json_obj(
                2,
                &[
        ...
        AgentNotification::ThinkingChanged { enabled, budget } => Notification::new(
            "agent.thinking_changed",
            json_obj(
                2,
                &[
        ...
        AgentNotification::CacheBreakSet => Notification::new(
            "agent.cache_break_set",
        ...
        AgentNotification::SwarmTeamDeleted { team_name } => Notification::new(
            "swarm.team_deleted",
            json_obj(1, &[("team_name", Value::String(team_name.clone()))]),
        ),
        ...
        AgentNotification::ToolSelected { tool_name } => Notification::new(
            "agent.tool_selected",
            json_obj(1, &[("tool_name", Value::String(tool_name.clone()))]),
        ),
        AgentNotification::ConflictDetected { file_path, agents } => Notification::new(
...
mod tests {
    ...
    fn parse_submit() {
        let req =
            parse_request("agent.submit", Some(serde_json::json!({"text": "hello"}))).unwrap();
    ...
    fn parse_submit_missing_text_field() {
        let err =
            parse_request("agent.submit", Some(serde_json::json!({"other": "val"}))).unwrap_err();
        assert_eq!(err.code, error_codes::INVALID_PARAMS);
    }
    ...
    fn parse_compact_with_instructions() {
        ...
        .unwrap();
        assert!(
            matches!(req, AgentRequest::Compact { instructions: Some(i) } if i == "Keep API calls")
        );
    }
    ...
    fn parse_set_model() {
        let req =
            parse_request("agent.setModel", Some(serde_json::json!({"model": "opus"}))).unwrap();
    ...
    fn parse_session_commands() {
        assert!(matches!(
            parse_request("session.save", None).unwrap(),
            AgentRequest::SaveSession
        ));
        assert!(matches!(
            parse_request("session.status", None).unwrap(),
            AgentRequest::GetStatus
        ));
        assert!(matches!(
            parse_request("session.shutdown", None).unwrap(),
    ...
    fn parse_mcp_list() {
        assert!(matches!(
            parse_request("mcp.listServers", None).unwrap(),
    ...
    fn notification_text_delta() {
        ...
        let text = jsonrpc.params.unwrap()["text"]
            .as_str()
            .unwrap()
            .to_string();
        assert_eq!(text, "hi");
    ...
    fn notification_error() {
        let notif = AgentNotification::Error {
            code: clawed_bus::events::ErrorCode::ApiError,
    ...
    fn notification_mcp_server_list() {
        let notif = AgentNotification::McpServerList {
            servers: vec![clawed_bus::events::McpServerInfo {
                name: "test".into(),
                tool_count: 3,
                connected: true,
            }],
    ...
    fn parse_session_load_valid() {
        ...
        .unwrap();
        assert!(
            matches!(req, AgentRequest::LoadSession { session_id } if session_id == "sess-123")
        );
    }
    ...
    fn parse_mcp_connect_valid() {
        ...
        .unwrap();
        if let AgentRequest::McpConnect {
            name,
            command,
            args,
            env,
    ...
    fn parse_send_message_valid() {
        ...
        .unwrap();
        assert!(
            matches!(req, AgentRequest::SendAgentMessage { agent_id, message }
    ...
    fn notification_compact_events() {
        ...
        let complete =
            notification_to_jsonrpc(&AgentNotification::CompactComplete { summary_len: 200 });
